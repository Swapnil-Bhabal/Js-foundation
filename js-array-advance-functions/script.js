const inventors = [
  ...
  const lifeOfInventors = inventors.sort((a,b) => (a.passed - a.year) - (b.passed - b.year));
  console.log(lifeOfInventors);


   // 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
    // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
    // const boulevards = document.querySelector('.mw-category-group');
    // const allList = Array.from(boulevards.querySelectorAll('a'));
    // const links = allList
    //                 .map(link => link.textContent)
    //                 .filter(finalName => finalName.includes('de'));
    


    // 7. sort Exercise
    // Sort the people alphabetically by last name
    const sortedPeopleNames = people.sort((last, next) => {
        const [aLast, aFirst] = last.split(', ');
        const [bLast, bFirst] = next.split(', ');
        return aLast > bLast ? -1 : 1;
    })

    console.log(sortedPeopleNames);

    // 8. Reduce Exercise
    // Sum up the instances of each of these
    const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk', 'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck' ];

    const sumUp = data.reduce((obj, items) => {
        if(!obj[items]) {
            obj[items] = 0;
        }
        obj[items]++;
